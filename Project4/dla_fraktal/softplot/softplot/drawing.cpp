...
void DrawSquarefloat(float cx, float cy, float r, int num_segments,int filled)
{
    float theta,c,s,t;
    ...
    //draw a filled circle
        ...
        // gluPerspective(100.0, 10.0, -1, 1);
        glBegin(GL_POLYGON);
            glVertex3f(cx, cy, -10);
            glVertex3f(cx+r, cy, -10);
            glVertex3f(cx+r, cy+r, -10);
            glVertex3f(cx, cy+r, -10);
        glEnd();
}
...
void draw_all_particles(int frame)
{
    ...
    for(i=0;i<global.N_objects;i++)
        ...
        if (inside_zoomed_box(x,y))
            ...
            DrawSquarefloat(x,y,global.radius_particle, 36,1);
            }
        }
}
